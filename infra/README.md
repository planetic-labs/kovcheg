...
Run both a latest-from-zero scenario and the compatible migration boundaries `0001 → 0002 → 0003 → 0004 → 0005` with:
...
The smoke script uses its own uniquely named Compose project. It builds the four application images, waits for all eight default containers, verifies the exact service, network, and loopback-port sets, checks that the service network is internal, proves the relay is unreachable from the published entrypoint, calls the documented routes from the host, validates dependency-aware API readiness and disabled-runtime Auth liveness, tests correlation IDs, and inspects production runtime contents and build provenance. Its cleanup cannot address a developer project's volume.
