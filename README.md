...
Migration `0009_persona_authorization.sql` adds a fail-closed internal authorization function for one current personal application session, its exact operator account, and one active granted system persona. The general runtime can execute the function but still has no direct grant or auth-state table access. Its internal API seam accepts an existing PostgreSQL transaction so a later authorized action can use the same transaction without changing current message, realtime, outbox, or browser contracts.
...
The production-shaped application forwards only the expected host-only application-session cookie to A2 and uses the returned server principal for chat listing, message creation, and history authorization.
