...
`pnpm deployment:verify` statically validates the image-only deployment contract, env schema, migration ordering, loopback/network isolation, volume classes, and resource ceilings. `pnpm deployment:smoke` builds and inspects all six repository-owned target images for `linux/amd64`, then runs the same source and Compose contract on the Docker daemon's native supported architecture. This avoids treating cross-architecture emulation as application evidence while still proving target-image provenance, fail-closed and positive session contracts, resource limits, and cleanup. Neither command publishes an image or creates an external route.
