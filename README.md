...
Migration `0016_auth_email_passkey_variant_e.sql` is the current email/passkey authentication boundary. It terminalizes historical personal-gate state, removes its runtime and administration privileges, rejects gate-sourced sessions, and exposes neutral email-challenge issuance only for a matching pre-created active account. Every locally valid submitted email receives the same code-state response, while only an issued challenge can deliver a code or create a session; missing, invalid, throttled, unknown, and deactivated outcomes remain non-disclosing. Server matching is case-insensitive after outer trim, while the client preserves the entered case, dots, and `+tag` for display and edit-back. Discoverable Conditional passkey authentication requires user verification and no email prelookup; voluntary registration requires an active application session and supports multiple synchronized credentials. Cache and rate-limit dependencies fail closed.

Migration `0017_oidc_application_session.sql` is the latest migration. It adds one protected, one-time OIDC-to-application-session bridge for an existing active account. The web BFF exchanges Authorization Code with mandatory PKCE S256 server-side, verifies issuer, audience, nonce, signature and token lifetime, and sends the opaque provider access token only to the internal Auth seam. Auth independently binds that token to the exact configured public `openid` client before creating a bounded application session. PostgreSQL stores only an HMAC verifier of the source token so replay fails closed; unknown, deactivated, disallowed, malformed and mismatched identities create neither an account nor a session and receive the same neutral BFF failure.
...
`pnpm deployment:verify` statically validates the image-only deployment contract, env schema, migration and fail-closed OIDC-client registration ordering, separate loopback-only application/issuer seams, file-backed BFF state key, internal-only session bridge, volume classes, and resource ceilings. Registration adds only missing public client metadata, accepts an exact existing match, and stops on conflicts without persisting client secrets. `pnpm deployment:smoke` builds and inspects all six repository-owned target images for `linux/amd64`, then runs the same source and Compose contract on the Docker daemon's native supported architecture. It proves an exact dual-host Authorization Code + PKCE S256 flow for a pre-created active synthetic account, including host-bound cookies, server-side exchange, one application session and replay rejection. This avoids treating cross-architecture emulation as application evidence while still proving target-image provenance, fail-closed and positive session contracts, resource limits, and cleanup. Neither command publishes an image or creates an external route.
